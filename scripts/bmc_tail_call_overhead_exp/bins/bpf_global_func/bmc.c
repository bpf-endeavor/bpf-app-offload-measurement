...
int main(int argc, char *argv[])
{
	...
	struct bmc_bpf* skel = bmc_bpf__open();
	if (!skel) {
		fprintf(stderr, "Error: bmc_bpf__open failed\n");
		return 1;
	}
	...
	if (err) {
		fprintf(stderr, "Error: bmc_bpf__load failed\n");
		return 1;
	}

	map_progs_xdp_fd = bpf_map__fd(skel->maps.map_progs_xdp);
	if (map_progs_xdp_fd < 0) {
		fprintf(stderr, "Error: bpf_object__find_map_fd_by_name failed\n");
		return 1;
	}

	map_progs_tc_fd = bpf_map__fd(skel->maps.map_progs_tc);
...
retry:
	...
	map_stats_fd = bpf_map__fd(skel->maps.map_stats);
	if (map_stats_fd < 0) {
		fprintf(stderr, "Error: bpf_object__find_map_fd_by_name failed\n");
		return 1;
	}
	...
	for (int i = 0; i < interface_count; i++) {
		if (bpf_xdp_attach(interfaces_idx[i], xdp_main_prog_fd, xdp_flags, 0) < 0) {
	...
	unsigned int curr_prog_id;

	for (int i = 0; i < interface_count; i++) {
		if (!bpf_xdp_query_id(interfaces_idx[i], XDP_FLAGS_DRV_MODE, &curr_prog_id)) {
			if (curr_prog_id) {
				bpf_xdp_detach(interfaces_idx[i], XDP_FLAGS_DRV_MODE, NULL);
				fprintf(stderr, "XDP program detached from interface: %d\n", interfaces_idx[i]);
			}
		}
	}
