...
# KEY 1 --> Small (in BMC)
WORKLOAD_DESC_BG="--records=1 -K 200 -V 1000"
LOAD_RATE=100000
...
for i in $(seq $REPEAT); do
	...
	taskset -c 7 $MUTILATE_DIR/mutilateudp \
		...
		$WORKLOAD_DESC_BG \
		--server=$SERVER_HOST:$M1_PORT --noload --connections=4 \
		--measure_connections=1 --measure_qps=1000 \
		--agent=localhost -p '5556' &>> /tmp/m1_result.txt &
	# --save=/tmp/m1_lat_samples_$i.txt \

	sleep 1
	taskset -c 11 $SOCKET_APP_BENCH &>> /tmp/sock_app_bench.txt &
