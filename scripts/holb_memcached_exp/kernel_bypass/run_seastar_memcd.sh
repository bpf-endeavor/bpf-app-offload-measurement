...
main() {
	...
	case $mode in
		...
		bmc)
			...
			load_xdp_prog=",xdp_prog=$bmc_binary"
			;;
	...
	extra_flags="$extra_flags --af-xdp-opts iface=$NET_IFACE,start_queue=$af_xdp_queue,queue_count=$count_cores""${load_xdp_prog},busy_budget=$busy_budget"
